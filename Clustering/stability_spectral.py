...
def compute(train : np.ndarray[np.float64], 
    ...
    clustering = SpectralClustering(affinity = "rbf", gamma = 1 / (2 * sigma), assign_labels="cluster_qr")
    ...
    if rank == 0:
        ...
        results.to_csv(f"{output_dir}/spectral_stability_results_{min_clusts}_{max_clusts}.csv")
        print("end")
